...
export default function LatestNews () {
  ...
  const [mainFeaturedNews, setMainFeaturedNews] = useState([]); // Use state to store featured news

  useEffect(() => {
    axiosInstance
      .get('/posts?term_type=news&per_page=19')
      .then(response => {
    ...
    axiosInstance
      .get('/posts?term_type=news&per_page=1&order_by=id:desc&is_featured=Yes')
      .then(response => {
        // Filter news where is_featured is not null (i.e., featured news)
        //console.log(response);        
        setMainFeaturedNews(response.data.data) // Update state with filtered data        
        setLoading(false) // Set loading to false when data is fetched
      })
      .catch(error => {
        setError(error) // Set error state if request fails
          ...
          <div className='lg:col-span-3'>
            <div className='grid grid-flow-row lg:grid-cols-4 lg:grid-flow-col lg:gap-4'>
              <div className='x-5 lg:col-span-3'>
                <div className='grid grid-flow-row lg:grid-cols-3 lg:grid-flow-col lg:gap-4'>
                  <Featured />
                  {mainFeaturedNews.length > 0 ? (
                    <div className='order-1 lg:col-span-2'>
                      <Link href={'/news/' + mainFeaturedNews[0].slug}>
                        <Image
                          src={mainFeaturedNews[0]?.featured_image} // Path relative to the public directory
                          alt={mainFeaturedNews[0]?.name}
                          width={800}
                          height={450}
                      ...
                      <div className='pt-2'>
                        <h2 className='text-4xl'>
                          <Link href={'/news/' + mainFeaturedNews[0].slug}>
                            {mainFeaturedNews[0].name}
                          </Link>
                        </h2>
                        <p>
                          <Link href={'/news/' + mainFeaturedNews[0].slug}>
                            {truncate(mainFeaturedNews[0].description, 240)}
                          </Link>
                        </p>
                      </div>
                    </div>
