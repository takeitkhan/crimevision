...
export default function Bangladesh () {
  ...
  useEffect(() => {
    ...
    const fetchBangladeshNews = axiosInstance.get(
      '/posts?term_type=news&category_slug=town_village&order_by=desc&per_page=11'
    )
    const fetchMaxVisitedNews = axiosInstance.get(
      '/posts?term_type=news&category_slug=town_village&order_by=visitor_count:desc&per_page=8'
    )
