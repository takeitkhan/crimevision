...
export default function News({ params }) {
    ...
    const [news, setNews] = useState(null);           // State to store news
    const [categories, setCategories] = useState([]); // State to store categories
    const [similarNews, setSimilarNews] = useState([]); // State to store similar news
    ...
    const fetchNewsData = async (retryCount = 3, delay = 2000) => {
        const cachedNews = localStorage.getItem(`news_${slug}`);
        if (cachedNews) {
            setNews(JSON.parse(cachedNews));
            setLoading(false);
        } else {
            try {
                const response = await axiosInstance.get(`/post?term_type=news&order_by=desc&slug=${slug}`);
                localStorage.setItem(`news_${slug}`, JSON.stringify(response.data.data));
                ...
                if (error.response && error.response.status === 429 && retryCount > 0) {
                    ...
                    setTimeout(() => fetchNewsData(retryCount - 1, delay * 2), delay);
                } else {
                    setError(error);
                    setLoading(false);
                }
    ...
    const fetchSimilarNews = debounce(async () => {
        try {
    ...
    }, 300); // Debounced request to avoid frequent API calls
    ...
    useEffect(() => {
        if (news) {
            ...
            updateOrCreateMetaTag('og:description', news.description || "Crime Vision");
            updateOrCreateMetaTag('og:image', news.featured_image || "/default-image.jpg"); // Use a default image if needed            
                                ...
                                <div className="basis-[70%]">
                                    ...
                                    <div className="flex flex-col md:flex-row items-center justify-between">
                                        <div className="pt-4 pb-10">
                                            ...
                                            <p>প্রকাশ: {news.post_date} | {news.post_time} | আপডেট: {news.update_post_date} | {news.update_post_time}</p>
                                        </div>
                                    ...
                                    <div>
                                        <div className="flex" id="printableDiv">
                                            <div className="lg:basis-[100%] flex flex-col gap-10" dangerouslySetInnerHTML={{ __html: news.description }}>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        {similarNews.length > 0 ? (
                                            similarNews.map((item, index) => (
                                                <div key={index} className="flex justify-between gap-4">
                                                    ...
                                                    <div className='basis-1/2'>
                                                        <Link className='basis-1/2 inline-block' href={`/news/${item.slug}`}>
