...
export default function CategoryPage() {
    ...
    useEffect(() => {
        ...
        var url = slug == 'latests' ? '/posts?term_type=news&per_page=13&order_by=desc&page=1' : `posts?term_type=news&per_page=10&category_slug=${slug}&page=1`;
    ...
    const loadMoreNews = () => {
        setLoadingMore(true);
        var url = slug == 'latests' ? `/posts?term_type=news&per_page=13&order_by=desc&page=${page + 1}` : `posts?term_type=news&per_page=10&category_slug=${slug}&page=${page + 1}`;
        ...
        <section className='py-10'>
            <div className='container mx-auto text-black px-3'>
                ...
                <div>
                    ...
                    <h1 className='text-3xl md:text-4xl text-orange-500'>{slug == 'latests' ? 'সর্বশেষ' : category.name}</h1>
                            ...
                            <div key={index} className='flex flex-row gap-10'>
                                <div className='basis-1/2'>
                                    ...
                                    <p className='text-[16px] group-hover:text-black group-hover:font-medium'>
                                        <Link href={'/news/' + item.slug} className='flex flex-col md:flex-row md:items-center gap-4 group'>{truncate(item.description, 100)}</Link>
                                    </p>
                                </div>
                                <div className='basis-1/2'>
                                    <Link href={'/news/' + item.slug} className='flex flex-col md:flex-row md:items-center gap-4 group'>
                                        <Image src={item.featured_image} className='w-full rounded-md' width={300} height={300} alt={item.name} />
                                    </Link>
                                </div>
                            </div>
                        ))}
